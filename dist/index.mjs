...
import fs__default, { writeFileSync, existsSync, readFileSync, statSync, mkdirSync, copyFileSync } from 'fs-extra';
import minimist from 'minimist';
import serveStatic from 'serve-static';
import { writefile, persistentCache, bindProcessExit } from 'sbg-utility';
import * as path from 'upath';
import path__default, { join, dirname } from 'upath';
...
import require$$2 from 'bluebird';
...
const getConfig = function (hexo, _key = 'config-hexo-seo') {
  const defaultOpt = {
    ...
    sitemap: false,
    theme_dir: path__default.join(process.cwd(), 'themes', String(hexo.config.theme || 'landscape')),
    source_dir: path__default.join(process.cwd(), String(hexo.config.source_dir || 'source')),
    post_dir: path__default.join(process.cwd(), String(hexo.config.source_dir || 'source'), '_posts')
  };
  const seo = hexo.config.seo;
  writeFileSync(path__default.join(__dirname, '_config_data.json'), JSON.stringify(seo, null, 2));
...
const coreCache = new persistentCache({
  base: tmpFolder,
  persist: true,
  memory: false,
...
class CacheFile {
  ...
  constructor(hash = null) {
    ...
    this.dbFile = path__default.join(tmpFolder, 'db-' + hash + '.json');
...
function dump(filename, ...obj) {
  ...
  const filePath = path__default.join(process.cwd(), '/tmp/hexo-seo/dump', hash);
  // truncate directory on first time
  if (!('dump' in firstIndicator)) {
    rimrafSync(filePath);
    firstIndicator['dump'] = true;
  }
  if (!fs.existsSync(path__default.dirname(filePath))) {
    fs.mkdirSync(path__default.dirname(filePath), {
      recursive: true
    });
  }
  let buildLog = '';
  ...
  console.log(`dump results saved to ${path__default.resolve(filePath)}`);
}
...
async function minifyJS(str, options) {
  ...
  const path0 = fs__default.existsSync(str) ? str : 'inline';
  if (path0 !== 'inline') {
    str = fs__default.readFileSync(path0).toString();
  }
  try {
    const result = await minify(str, minifyOptions);
    if (result.code && result.code.length > 0) {
...
function initSitemap(type) {
  if (!sitemapGroup[type]) {
    ...
    const doc = create(readFileSync(sourceXML).toString());
    ...
    if (isPagePost) {
      ...
      bindProcessExit('writeSitemap', () => {
        if (isYoastActive) {
          ...
          if (!existsSync(dirname(destXSL))) mkdirSync(dirname(destXSL), {
            recursive: true
          });
          ...
          writefile(destPostSitemap, create(sitemapGroup['post']).end({
            prettyPrint: true
          }));
          ...
          writefile(destPageSitemap, create(sitemapGroup['page']).end({
            prettyPrint: true
          }));
...
function generateSitemapIndex(hexoinstance = null) {
  ...
  const sitemapIndexDoc = create(readFileSync(sourceIndexXML).toString());
  ...
  writefile(destTagSitemap, create(sitemapGroup['tag']).end({
    prettyPrint: true
  }));
  ...
  writefile(destCategorySitemap, create(sitemapGroup['category']).end({
    prettyPrint: true
  }));
  ...
  writefile(destIndexSitemap, create(sitemapIndex).end({
    prettyPrint: true
  }));
...
async function HexoSeoHtml(content, data) {
  ...
  if (cache.isFileChanged(md5FileExports.md5(path0)) || isDev) {
    ...
    if (cfg.js.concat === true) {
      ...
      const filename = 'concat-' + md5FileExports.md5(path__default.basename(path0));
      ...
      for (let i = 0; i < scripts.length; i++) {
        ...
        */
        ...
        if (typeof src === 'string' && src.trim().length > 0) {
          ...
          path__default.join(cfg.theme_dir, 'source'),
          // find from node_modules directory
          path__default.join(process.cwd(), 'node_modules'),
          // find from our plugins directory
          path__default.join(process.cwd(), 'node_modules/hexo-shortcodes'),
          // find from source directory
          cfg.source_dir,
          // find from post directory
          cfg.post_dir,
          // find from asset post folder
          path__default.join(cfg.post_dir, path__default.basename(path0))].map(dir => path__default.join(dir, src));
          const sources = originalSources.filter(fs__default.existsSync);
          if (sources.length > 0) {
            try {
            ...
            hexo.log.error(logconcatname, 'log', writefile(path__default.join(tmpFolder, 'logs', filename + '.log'), originalSources).file);
          }
        } else {
          // push inner
      ...
      const filePathWithoutExt = path__default.join(tmpFolder, 'html', filename);
      const jsFilePath = path__default.join(buildFolder, 'hexo-seo-js', filename) + '.js';
      ...
      hexo.log.debug(logconcatname, 'written', writefile(path__default.join(process.cwd(), hexo.config.public_dir, newsrc), scriptContent).file);
...
function HexoSeo(hexo) {
  ...
  hexo.extend.filter.register('after_clean', function () {
    ...
    if (fs__default.existsSync(tmpFolder)) fs__default.rmSync(tmpFolder, {
      recursive: true,
      force: true
    });
    if (fs__default.existsSync(buildFolder)) fs__default.rmSync(buildFolder, {
      recursive: true,
      force: true
    });
  });
  ...
  hexo.extend.generator.register('hexo-seo-js', () => {
    ...
    for (let i = 0; i < concatRoutes.length; i++) {
      ...
      wrap.push({
        path,
        data: () => fs__default.createReadStream(absolute)
      });
    }
    return wrap;
  });
  // Register build folder to serving statically
  if (!fs__default.existsSync(buildFolder)) fs__default.mkdirSync(buildFolder, {
    recursive: true
  });
