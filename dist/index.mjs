...
import fs, { existsSync, readFileSync, statSync, mkdirSync, copyFileSync } from 'fs-extra';
...
import { writefile, persistentCache, array_unique as array_unique$1, md5, normalizePathUnix, bindProcessExit } from 'sbg-utility';
import path, { join, dirname } from 'upath';
import crypto from 'crypto';
import { parse } from 'node-html-parser';
import parseUrl from 'url-parse';
import NodeCache from 'node-cache';
import * as hexoLog from 'hexo-log';
import { logger as logger$1 } from 'hexo-log';
import fs$1 from 'fs';
...
async function hexoSeoSearch(args, callback) {
  try {
    ...
    config.search.type = array_unique$1(config.search.type);
    ...
    hexo.log.info(`[hexo-seo] Local search saved to ${paths.map(file => normalizePathUnix(file).replace(normalizePathUnix(hexo.base_dir), '')).join(', ')}.`);
...
typeof hexo !== "undefined" ? hexo.log : logger$1({
  debug: false,
  silent: false
});
/**
...
async function HexoSeoHtml(content, data) {
  ...
  if (cache.isFileChanged(md5FileExports.md5(path0)) || isDev) {
    ...
    if (cfg.js.concat === true) {
        ...
        */
        ...
        if (typeof src === 'string' && src.trim().length > 0) {
          ...
          if (sources.length > 0) {
            ...
            hexo.log.error(logconcatname, 'log', writefile(path.join(tmpFolder, 'logs', filename + '.log'), originalSources).file);
          }
        } else {
          // push inner
      ...
      hexo.log.debug(logconcatname, 'written', writefile(path.join(process.cwd(), hexo.config.public_dir, newsrc), scriptContent).file);
